...
If line 3 was changed to `c = a.uniq!`, all 3 variables would reference `[1, 2, 3]`. This is because `uniq!` is a destructive method; therefore, when called on `a`, its value would be mutated. Because `b` is referencing the same object as `a`, its value would also be affected by the mutation.